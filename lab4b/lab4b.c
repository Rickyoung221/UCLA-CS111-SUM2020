...
*/
const char* usage =
    "Usage: --scale={F(Farenheight), C(Celcius)}; --period=# Temperature] ; --log = filename ";
...
int main(int argc, char *argv[]) {
    ...
    while (run_flag) {
        ...
        if (difftime(raw_time, raw_start) == period) {
            ...
            if (report) {
                ...
                if (log_flag) {
                    if (write(log_file, buffer, strlen(buffer)) < 0){
                        error_handling("ERRO! Cannot write to log file. \n");
                    }
                }
    ...
    if (log_flag) {
        if (write(log_file, exit_info, strlen(exit_info)) < 0)
